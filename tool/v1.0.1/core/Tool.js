...
window.$tool = {
  ...
  /**
   * @description 将目标内容高亮后返回
   * @param {string} target
   * @param {string or regexp} keyword
   * @param {html} wrap  包裹元素
   * @returns {string} 高亮后内容
   * 
   * const target = '竞网智赢网络科技有限公司'
   * const keyword = '竞网'
   * console.log(highlight(target, keyword, `<span style="color: red;">${keyword}</span>`))
   */
  highlight(target, keyword, wrap) {
    let regexp = new RegExp(keyword, 'g')
    return target.replace(regexp, wrap)
  },
}
