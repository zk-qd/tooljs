window.ObjectKit = {
  ...
  StateModel: function(states) {
    // 安全模式
    if (new.target !== StateModel) {
      return new StateModel(states);
    }
    let currentState = {};
    // 控制类
    let constrol = {
  ...
  CommandModel: function(command = {}) {
    if (new.target !== CommandModel) return new CommandModel(command);
    return {
      /**
       * *执行命令方法
       * execute(args)
  ...
  Flyweight: function(logic = {}) {
    if (new.target !== ObjectKit.Flyweight)
      return new ObjectKit.Flyweight(logic);
    return {
      /**
       * *执行逻辑的方法
       * call(args)
